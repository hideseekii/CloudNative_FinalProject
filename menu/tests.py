...
class MenuViewTests(TestCase):
    ...
    def test_add_to_cart(self):
        response = self.client.post(reverse('menu:add_to_cart', args=[self.dish.id]))
    ...
    def test_remove_from_cart(self):
        ...
        response = self.client.post(reverse('menu:remove_from_cart', args=[self.dish.id]))
...
class StaffViewsTest(TestCase):
    ...
    def test_update_dish_view(self):
        response = self.client.post(reverse('menu:dish_edit', args=[self.dish.id]), {
            'name_zh': '改過的炒飯',
            'price': 120,
            'is_available': True,
        })
    ...
    def test_delete_dish_view(self):
        response = self.client.post(reverse('menu:dish_delete', args=[self.dish.id]))
        self.assertRedirects(response, reverse('menu:dish_list'))
